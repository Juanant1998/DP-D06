...
public class HandyWorkerService {
	...
	public FixUpTask getFixUpTaskByApplication(final Application application) {
		FixUpTask result = null;
		final Collection<FixUpTask> allfuts = this.futs.findAll();

		for (final FixUpTask f : allfuts)
			if (f.getApplications().contains(application))
				result = f;
