...
public class HandyWorkerService {
		...
		public Collection<FixUpTask> getFixUpTasksUserInvolved(){
			HandyWorker actual = uas.getHandyByUserAccount(LoginService.getPrincipal());
			Assert.isTrue(!actual.getIsBanned());
			Collection<FixUpTask> result = handyWorkerRepository.getFixUpTaskByHandyWorker(LoginService.getPrincipal());
			
			return result;
		}
		
		public Note createNoteInReport(Note n, Report r){
			HandyWorker actual = uas.getHandyByUserAccount(LoginService.getPrincipal());
			Assert.isTrue(!actual.getIsBanned());
			Collection<Note> x = r.getNotes();
			x.add(n);
			
			r.setNotes(x);
			
			Note result = ns.save(n);
			Report opt = rs.save(r);
			
			return result;
			
		}
		
		public Note writeCommentOnNote(Note n, String c){
			HandyWorker actual = uas.getHandyByUserAccount(LoginService.getPrincipal());
			Assert.isTrue(!actual.getIsBanned());
			actorservice.checkSpammer(c);
		...
		public Collection<Tutorial> getTutorials(UserAccount u) {
			Assert.isTrue(!actorservice.isActualActorBanned());
			this.checkAuthority();
		...
		public  Endorsement createEndorsement(Customer customer,Endorsement endor,FixUpTask n){
