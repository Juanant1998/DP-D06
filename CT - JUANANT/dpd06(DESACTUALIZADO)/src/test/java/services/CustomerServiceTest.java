...
public void testGetEndorsements(){
	super.authenticate("customer1");
	Collection<Endorsement> en = this.customerService.getEndorsments();
	Assert.notEmpty(en);
	super.authenticate(null);
	}
@Test
public void testUpdateEndorsement(){
	super.authenticate("customer1");
	Endorsement a = this.endorsementService.findOne(super.getEntityId("endorsement1"));
	this.customerService.updateEndorsement(a);
	Assert.notNull(a);	
	super.authenticate(null);
}
@Test
public void testCreateEndorsement(){
	super.authenticate("customer1");
	HandyWorker handy = this.handyWorkerService.findOne(super.getEntityId("handyWorker5"));
	Endorsement  end  = this.endorsementService.findOne(super.getEntityId("endorsement1"));
	FixUpTask    fut = this.fixUpTaskService.findOne(super.getEntityId("fixUpTask1"));
	Endorsement  res =	this.customerService.createEndorsement(handy, end,fut);
	Assert.isTrue(res.getComments().equals(end.getComments()));
	super.authenticate(null);
...
public void testDeleteEndorsement(){
	super.authenticate("customer1");
	Endorsement endor = this.endorsementService.findOne(super.getEntityId("endorsement1"));
     this.endorsementService.delete(endor);
     Assert.isNull(this.endorsementService.findOne(endor.getId()));
     super.authenticate(null);
